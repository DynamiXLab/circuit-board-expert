...
| **②** | **EN** | 高电平开启芯片(>1.5V)，低电平关闭(<0.7V)。**最高只能承受6V** | 拉低就没输出 |
| **③** | **COMP** | 补偿引脚。**RT8289GSP内部自带补偿**，此引脚悬空即可 | 悬空也没事（内部已补偿） |
| **④** | **FB** | 检测输出电压（内部基准**1.222V**） | 输出会失控 |
| **⑤** | **GND** | 连接GND和底部散热焊盘 | |
| **⑥** | **SW** | 接电感和续流二极管。**电压在0V和Vin之间高频跳变** | 能量传不出去 |
| **⑦** | **IN** | 主电源输入（**5.5~32V**） | 没电 |
| **⑧** | **SS** | 软启动。**RT8289GSP内部自带软启动**，此引脚悬空即可 | 悬空也没事（内部已软启） |
...
RT8289GSP的Buck降压电路由四个核心元件组成（和MP1584原理相同——**Buck拓扑是所有DC-DC降压芯片的共同基础**）：
...
| 元件 | 比喻 | 实际作用 |
|:----:|:----|---------|
| **① 开关管** | 水龙头的开关 | 以**500kHz**频率通断，把直流切成脉冲 |
| **② 电感L** | **飞轮**（有惯性） | 通电时储能，断电时释放，维持电流 |
| **③ 续流二极管** | 回水管道 | 开关断开时给电感电流提供回路 |
| **④ 输出电容Cout** | 大水桶 | 把脉冲电流"抚平"成平稳直流 |

> 500kHz的频率比MP1584的1.5MHz低——所以RT8289GSP需要的电感值大一些（22~33μH vs 10~15μH），但好处是开关损耗更小、效率略高。
...
> **与MP1584的核心区别**：RT8289GSP的开关频率是500kHz（MP1584是1.5MHz），频率低了3倍——所以每个开关周期的时间变长了，电感的充放电时间也更长，因此需要更大的电感值（22~33μH）来保持相同的纹波大小。
...
FB引脚检测输出电压，与内部基准**1.222V**比较：
- Vout太高 → 减小占空比 → SW导通时间缩短 → Vout降回来
- Vout太低 → 增大占空比 → SW导通时间加长 → Vout升上去

> 🔄 **这就是闭环控制！** 和LDO的原理一样——"睁眼走路"，不断看输出、调占空比。
...
| 变化点 | MP1584 | RT8289GSP | 好处 |
|:-------|:-------|:----------|:-----|
| COMP引脚 | 需要R4(10k)+C7(3.3nF) | **悬空！内部已补偿** | 省2个元件 ✅ |
| SS引脚 | 需要C5(0.1μF) | **悬空！内部已软启动** | 省1个元件 ✅ |
| 反馈基准 | 0.8V → R1=52.3kΩ | **1.222V → R1=31.6kΩ** | 基准更高，抗噪声更好 |
| 电感 | 10~15μH | **22~33μH** | 频率低，需要更大电感 |
| 续流二极管 | SS34(3A) | **SS54(5A)** | 电流更大 |
