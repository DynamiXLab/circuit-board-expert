...
## 9. PCB布局小技巧——留意电流的回流路径

> ⚡ 画PCB时，很多人只盯着"从哪儿到哪儿"的走线，却忽略了电流怎么**回来**——回流路径和信号路径同样重要。

### 为什么回流路径很重要？

任何电路中的电流都是**一个完整的回路**：从电源正极出发，经过元件，最后回到电源负极。如果回流路径设计得不好，会出现三个问题：

| 问题 | 后果 |
|:----|:------|
| **回路面积大** | 产生电磁辐射（EMI），干扰板上其他电路 |
| **回流和信号走线不平行** | 产生额外的寄生电感，导致电压尖峰 |
| **不同回路共用一段地线** | 产生"地弹"——大电流的回流会干扰小信号的地电位 |

### Buck电路的关键回流路径

在RT8289GSP的Buck电路中，两个回路的面积需要特别注意：

```
回路1（导通时）：Vin → C1/C2 → RT8289GSP的IN
                                  ↓
                              内部开关管导通
                                  ↓
                              SW → L1 → C3/C4 → 负载 → GND
...
```
① 输入电容C1/C2的GND焊盘 → 打多个过孔到底层GND铺铜
   目的：让回流路径最短、回路面积最小

② SW节点周围的GND铺铜 → 不要打断
   目的：大电流开关回路的回流路径保持连续

③ FB反馈走线的下方 → 有一层完整的GND平面
   目的：FB是敏感信号，需要GND平面做屏蔽
```

![RT8289GSP回流路径示意](RT8289GSP-return-path.png)

*↑ 绿色虚线表示导通时的电流回路，红色虚线表示关断时(续流)的电流回路，回路面积越小EMI越低*

> 记住一句话：**走线只是"去程"，铺铜是"返程"**——去程和返程构成的回路面积越小，电路的电磁兼容性越好。
