...
其中 **D（占空比）** = 开关管导通时间 ÷ 整个周期

但实际电路中，占空比不是人手工设定的——而是由**反馈环路**自动调节的。

### 反馈环路怎么算输出电压？

FB引脚检测输出电压的分压值，与内部基准**1.222V**比较：

```
Vout太高 → 减小占空比 → 开关管导通时间变短 → Vout降下来
Vout太低 → 增大占空比 → 开关管导通时间变长 → Vout升上去
```

输出5V需要的反馈分压电阻计算，见第**4.3节**。

> 🔄 **这就是闭环控制！**——"睁眼走路"，不断看输出、调占空比。
