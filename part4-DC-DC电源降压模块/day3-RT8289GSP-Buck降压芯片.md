...
| 功能点 | 说明 | 好处 |
|:-------|:----|:-----|
| 内部补偿 | COMP引脚悬空即可，无需外部RC网络 | 省2个元件 ✅ |
| 内部软启动 | SS引脚悬空即可，无需外部电容 | 省1个元件 ✅ |
| 反馈基准 | 1.222V基准电压，抗噪声能力强 | 输出更稳定 |
| 电感值 | 22~33μH（500kHz频率下） | 开关损耗低 |
| 续流二极管 | SS54(5A/40V) 肖特基 | 电流余量大 |
...
RT8289GSP的开关频率是**500kHz**，每个开关周期的时间较长，电感的充放电时间也较长→需要较大的电感值来控制纹波。
...
> RT8289GSP是高频开关电源，PCB布局的四条铁律需要严格遵守。
...
- **SOP-8 EP封装**：底部散热焊盘焊接到GND
- **SS54二极管** 比SS34大，阴极（有横线端）接SW，阳极接GND
- 电感值22~33μH对应的CD75或CD105封装在焊接时注意对准即可
...
```
回路1 — 开关管导通时：
Vin → C1/C2 → RT8289GSP的IN → 内部开关管 → SW → L1 → C3/C4 → 负载 → GND → 回C1/C2
 ↕ 这个回路面积要尽量小！

回路2 — 开关管关断时（续流）：
L1（续流）→ C3/C4 → 负载 → GND → 续流二极管D1 → 回L1
 ↕ 这个回路面积也要尽量小！
```
...
```
① 输入电容C1/C2的GND焊盘 → 打多个过孔到底层GND铺铜 → 让回流路径最短
② SW节点周围的GND铺铜 → 不要打断 → 保持大电流回流路径连续
...
> 记住一句话：**走线是"去程"，铺铜是"返程"**——去程和返程构成的回路面积越小，电路的电磁兼容性越好。
